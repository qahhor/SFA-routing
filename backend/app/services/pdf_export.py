...
class PDFExporter:
    ...
    def export_daily_plan(
        ...
        elements.append(Paragraph(
            f"Date: {plan_date.strftime('%d.%m.%Y')} ({plan_date.strftime('%A')})",
        ...
        if visits:
            # Table header
            table_data = [[
                "#", "Time", "Client", "Address", "Distance", "Duration"
            ]]
    ...
    def export_weekly_plan(
        ...
        for plan in daily_plans:
            ...
            elements.append(Paragraph(
                f"{plan.get('day_of_week', '')} - {plan.get('date', '')}",
    ...
    def export_delivery_route(
        ...
        # Vehicle info
        elements.append(Paragraph(
            f"Vehicle: {vehicle_name} ({license_plate})",
            self.styles['Normal']
        ))
        elements.append(Paragraph(
            f"Date: {route_date.strftime('%d.%m.%Y')}",
            self.styles['Subtitle']
        ))
        ...
        if stops:
            table_data = [[
                "#", "ETA", "Client", "Address", "Weight", "Status"
            ]]
