...
async def create_order(
    ...
    existing = await db.execute(select(DeliveryOrder).where(DeliveryOrder.external_id == data.external_id))
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=400, detail=f"Order with external_id '{data.external_id}' already exists")
...
async def list_orders(
    ...
    if date_from:
        query = query.where(DeliveryOrder.time_window_start >= datetime.combine(date_from, datetime.min.time()))

    if date_to:
        query = query.where(DeliveryOrder.time_window_end <= datetime.combine(date_to, datetime.max.time()))
...
async def optimize_delivery_routes(
    ...
    orders_result = await db.execute(
        select(DeliveryOrder).options(selectinload(DeliveryOrder.client)).where(DeliveryOrder.id.in_(request.order_ids))
    )
    ...
    # Get vehicles
    vehicles_result = await db.execute(select(Vehicle).where(Vehicle.id.in_(request.vehicle_ids)))
    ...
    if len(vehicles) != len(request.vehicle_ids):
        ...
        raise HTTPException(status_code=400, detail=f"Vehicles not found: {missing}")
...
async def get_route(
    ...
    # Batch load all clients to avoid N+1 queries
    client_ids = {stop.order.client_id for stop in route.stops if stop.order and stop.order.client_id}
...
async def reoptimize_route(
    ...
    opt_result = await route_optimizer.optimize(
        orders=orders, vehicles=[vehicle], clients_map=clients_map, route_date=route.route_date
    )
